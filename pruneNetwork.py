...
class ModelDeploy(object):
    def __init__(self, y, output, config_params, lr,l2_flag):
        ...
        self.channel_tensor = []
        self._extendgroup = []
        self._extendgroup_coff = []
        for i in range(len(weight_list)-1):
            c_weight = tf.reduce_sum(tf.abs(weight_list[i]), axis=[0, 1, 2])
            n_weight = tf.reduce_sum(tf.abs(weight_list[i+1]), axis=[0, 1, 3])
            extragroup =c_weight+n_weight
            self._extendgroup.append(extragroup)
            l = c_weight.get_shape().as_list()[0]
            extragroup = tf.expand_dims(extragroup,0)
            # b = tf.layers.Dense(l/2,activation=tf.nn.relu)(extragroup)
            a_1= tf.Variable(tf.truncated_normal([l,l//2]))
            tf.add_to_collection("senet_weight_1", a_1)
            b_1 = tf.Variable(tf.truncated_normal([l//2]))
            c = tf.matmul(extragroup,a_1)+b_1
            c = tf.nn.relu(c)
            a_2 = tf.Variable(tf.truncated_normal([l//2,l]))
            tf.add_to_collection("senet_weight_2", a_2)
            b_2 = tf.Variable(tf.truncated_normal([l]))
            c = tf.nn.relu(tf.matmul(c,a_2)+b_2)
            b = tf.squeeze(c)
            b_max = tf.reduce_max(b)
            b_min = tf.reduce_min(b)
            b_mean = tf.reduce_mean(b)
            b = tf.abs((b-b_mean) / (b_max - b_min))* FLAGS.cg_value

            # b = tf.clip_by_value(b,0,1)
            # b = tf.clip_by_norm(b,0.02)
            self._extendgroup_coff.append(b)
        ...
        def extra_group(weight,coff,k):
            _extra = tf.constant(0, dtype=tf.float32)
            l = []
            t = weight.get_shape().as_list()[0]
            temp = t %k
            # 分组的列表
            for i in range(k):
                if temp > 0:
                    ...
                    l.append(t // k)
            weight = tf.sort(weight)
            s_weight = tf.split(weight,num_or_size_splits=l)
            for i in range(k):
                _extra += tf.sqrt(tf.reduce_sum(s_weight[i]*s_weight[i]))* coff*(k-i)
            return _extra
        ...
        def cross_group(weight1, weight2, weight_coff):
            ...
            # t = t1+t2
            tt = tf.reduce_sum(tf.sqrt(t1*t1+t2*t2)*weight_coff)
            # tt = tf.reduce_sum(tf.sqrt(t))
            # tt = tf.sqrt(tf.reduce_sum(t * t))
            # tt = tf.sqrt(tf.reduce_sum(t1*t1)+tf.reduce_sum(t2*t2))
            return tt
        ...
        if FLAGS.crossgroup == True:
            cg_list = weight_list
            # addd = tf.Variable(initial_value=0.0,trainable=False)
            # tf.add_to_collection("addaweight",addd)
            crossgroup_ = tf.constant(0, dtype=tf.float32)
            t = 1
            senet_1 = tf.get_collection("senet_weight_1")
            senet_2 = tf.get_collection("senet_weight_2")
            ...
            if FLAGS.model == "ResNet" :
                ...
                for i in range(len(cg_list) - 1):
                    ...
                    # crossgroup_ += cross_group(cg_list[i], cg_list[i + 1], t * config_params["crossgroup_para"])
                    # if i > 2:
                    #     crossgroup_ += cross_group(cg_list[i], cg_list[i + 1], self._extendgroup_coff[i])
                    # else:
                    #     crossgroup_ += cross_group(cg_list[i], cg_list[i + 1])
                    crossgroup_ += cross_group(cg_list[i], cg_list[i + 1], self._extendgroup_coff[i])

                    # crossgroup_ += (tf.reduce_sum(tf.abs(senet_1[i])) + tf.reduce_sum(tf.abs(senet_2[i]))) * 0.002
                    # crossgroup_ += extra_group(self._extendgroup[i],config_params["crossgroup_para"],3)
    ...
    def pass_classindex(self,classinedx):
        self.classdata =classinedx
    ...
    @property
    def extendgroup(self):
        return self._extendgroup
...
##聚类
def KMeans(data,k):
    index = np.argsort(np.array(data))
    def distabsolute(x, y):
        return np.abs(x - y)
    # 质心
    k_points = []
    for i in range(k):
        if i ==0 :
            k_points.append(data[index[0]])
        elif i == k-1:
            k_points.append(data[index[-1]])
        else:
            k_points.append(data[index[int( (len(data)*i)/ (k-1))]])

    def refram(data,k,k_points):
        ...
        for i in range(len(data)):
            ...
            cur_min = 0
            for j in range(k):
                d = distabsolute(data[i], k_points[j])
                if d < mindist:
                    cur_min = j
                    mindist = d

            classdata[cur_min].append(i)
        # print("----------------------")
        # for i in range(len(classdata)):
        #     temp_data = []
        #     for j in classdata[i]:
        #         temp_data.append(data[j])
        #     print(sorted(temp_data),end=",")
        #     print("centerid :",k_points[i],end=" ")
        #     print("")
        ## 重新规划
        center = []
        for i in range(k):
            ...
            restore_index = []
            for j in classdata[i]:
                re_data.append(data[j])
                restore_index.append(j)
            re_index = np.argsort(np.array(re_data))
            # if len(re_index) == 0:
            #     print(re_index)
            # print(re_index)
            center_index = re_index[int(len(re_index)/2)]
            center_index = restore_index[center_index]
            center.append(data[center_index])
        return center,classdata
    last_center = []
    for i in range(5):
        k_points, classdata = refram(data,k,k_points)
        if last_center == k_points:
            break
        last_center = k_points
    return classdata
...
def train_mode(config_params, sess, global_step, train_x, train_y, test_x, test_y, train_ops, cross_entropy, accuracy,
    ...
    for ep in range(start, total + 1):
        ...
        reg = use_flag * reg
        # if ep >= 50:
        #     _extendgroup = md.extendgroup
        #     extendgroup = sess.run(_extendgroup)
        #     new_cg = 0
        #     for t in range(len(extendgroup)):
        #         classindex = KMeans(extendgroup[t], 3)
        #         for i in range(3):
        #             e = 0
        #             for j in classindex[i]:
        #                 e += extendgroup[t][j] * extendgroup[t][j]
        #             new_cg += np.sqrt(e) * FLAGS.cg_value * (3 - i)
        #     # print("new add cg value: ", new_cg)
        #     add_value = tf.get_collection("addaweight")[0]
        #     sess.run(tf.assign(add_value, new_cg))
        # if ep == 90 or ep == 130:
